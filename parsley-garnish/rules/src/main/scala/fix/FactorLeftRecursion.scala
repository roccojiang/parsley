...
class FactorLeftRecursion(config: FactorLeftRecursionConfig) extends SemanticRule("FactorLeftRecursion") {
  ...
  private def unfold[NT](env: Map[Symbol, Parser], nonTerminal: Symbol)(implicit doc: SemanticDocument): UnfoldedProduction = {

    def unfold0[A](visited: Set[Symbol], nt: Parser): UnfoldedProduction = {

      def unfoldNonTerminal(sym: Symbol): UnfoldedProduction = {
        val tpe = TypeUtils.getParsleyType(sym)
      ...
      def unfoldAp[B, C](p: Parser, q: Parser): UnfoldedProduction = {
          val UnfoldedProduction(pe, pn, pl) = unfold0(visited, p)
          val UnfoldedProduction(qe, qn, ql) = unfold0(visited, q)
